//
//  UNDirect_Weight_Graph.h
//  NVM-Graph
//
//  Created by 江清月近人 on 2021/3/30.
//

#ifndef UNDirect_Weight_Graph_h
#define UNDirect_Weight_Graph_h
...
public:
    UNDirect_Weight_Graph(Arena* arena,Arena* AttrName,Arena* AttrIndex,Arena* AttrValue);
...
inline UNDirect_Weight_Graph::Node::Node(const int nid,const char* nodedata):Nid(nid),NidVNum(0),NextNode(0),Flag(1){
    size_t length=strlen(nodedata);
    if(length>=NodeDatalengthDef) length=NodeDatalengthDef-1;
    memcpy(Data,nodedata,length);
    Data[length]='\0';
}
inline void UNDirect_Weight_Graph::Node::SetData(const char *data){
    size_t length=strlen(data);
    memcpy(Data, data, length);
    Data[length]='\0';
}
inline bool UNDirect_Weight_Graph::Node::IsNbrNid(const int &nid)const{
    for(int i=0;i<NidVNum;++i){
        if(NidV[i]==nid) return true;
    }
    return false;
}
inline bool UNDirect_Weight_Graph::Node::AddNbrNid(const int& nid){
...
inline void UNDirect_Weight_Graph::Node::DeleteNbrNidN(const int& NodeN){
    for(int i=NodeN;i<NidVNum;++i){NidV[i]=NidV[i+1];}
    NidVNum--;
}
inline bool UNDirect_Weight_Graph::Node::DeleteNbrNid(const int& nid){
    for(int i=0;i<NidVNum;++i){
        if(NidV[i]==nid){
...
inline int UNDirect_Weight_Graph::NodeIter::GetDeg()const{
    int Deg=0;
    Node* temp=CurNode;
...
inline int UNDirect_Weight_Graph::NodeIter::GetInDeg()const{
    int InDeg=0;
    Node* temp=CurNode;
    while(temp->GetNextNodeAddre()!=0){
        InDeg+=temp->GetInDeg();
...
inline int UNDirect_Weight_Graph::NodeIter::GetOutDeg()const{
    int OutDeg=0;
    Node* temp=CurNode;
    while(temp->GetNextNodeAddre()!=0){
...
inline int UNDirect_Weight_Graph::NodeIter::GetNbrNid(const int &nodeN)const{
    int n=nodeN+1;
    Node* temp=CurNode;
...
inline int UNDirect_Weight_Graph::NodeIter::GetInNid(const int &nodeN)const{
    int n=nodeN+1;
    Node* temp=CurNode;
...
inline int UNDirect_Weight_Graph::NodeIter::GetOutNid(const int &nodeN)const{
    int n=nodeN+1;
    Node* temp=CurNode;
...
inline bool UNDirect_Weight_Graph::NodeIter::IsNbrNid(const int &nid)const{
    Node* temp=CurNode;
    while(temp->GetNextNodeAddre()!=0){
...
inline bool UNDirect_Weight_Graph::NodeIter::IsInNid(const int &nid)const{
    Node* temp=CurNode;
    while(temp->GetNextNodeAddre()!=0){
...
inline bool UNDirect_Weight_Graph::NodeIter::IsOutNid(const int &nid)const{
    Node* temp=CurNode;
    while(temp->GetNextNodeAddre()!=0){
...
UNDirect_Weight_Graph::UNDirect_Weight_Graph(Arena* arena,Arena* AttrName,Arena* AttrIndex,Arena* AttrValue):NodeTable(arena),NodeHash(32, 4),AttrE(AttrName,AttrIndex,AttrValue),MxNid(1),NodeNum(0),EdgeNum(0),FreeNodeLocation(0),FreeNodeNum(0){
...
int UNDirect_Weight_Graph::AddNode(const int& nid,const char* data){
    int newnid;
...
void UNDirect_Weight_Graph::DeleteNode(const uint64_t &location){
...
void UNDirect_Weight_Graph::DelEdgeOfNode(const uint64_t &location, const int &edgenid){
...
void UNDirect_Weight_Graph::DelNode(const int &nid){
    uint64_t location;
    ...
    do{
        for(int i=0;i<node->GetDeg();++i){
            ...
            if(NodeHash.Find(SrcNid, SrcLocation)){
                ...
                AttrE.DelAttrDat(SrcNid,nid,false);
                EdgeNum--;
            }
        }
...
UNDirect_Weight_Graph::Node* UNDirect_Weight_Graph::AddExistNode(const int& nid,const int& flag){
    Node* newnode;
    if(FreeNodeNum>0){
        uint64_t location=FreeNodeLocation;
        FreeNodeLocation=GetNodePtr(FreeNodeLocation)->GetNextNodeAddre();
...
void UNDirect_Weight_Graph::AddEdgeToNode(const uint64_t &location, const int &edgenid){
    uint64_t temp=location;
    Node* curnode=GetNodePtr(temp);
...
int UNDirect_Weight_Graph::AddEdge(const int &SrcNid, const int &DstNid){
...
int UNDirect_Weight_Graph::AddEdge2(const int &SrcNid, const int &DstNid){
    if(IsEdge(SrcNid, DstNid)) {return 0;}
    uint64_t location1,location2;
...
int UNDirect_Weight_Graph::AddEdge(const int& SrcNid,const int& DstNid,const int& AttrId,const int& Weight){
    uint64_t location1,location2;
    if(!NodeHash.Find(SrcNid, location1) || !NodeHash.Find(DstNid, location2)) {return -1;}
    if(IsEdge(SrcNid, DstNid)) {return 0;}
    AddEdgeToNode(location1, DstNid);
...
int UNDirect_Weight_Graph::AddEdge2(const int&SrcNid, const int& DstNid,const int& AttrId,const int& Weight){
    if(IsEdge(SrcNid, DstNid)) {return 0;}
    uint64_t location1,location2;
    if(!NodeHash.Find(SrcNid, location1)){AddNode(SrcNid);NodeHash.Find(SrcNid, location1);}
    if(!NodeHash.Find(DstNid, location2)){AddNode(DstNid);NodeHash.Find(DstNid, location2);}
...
void UNDirect_Weight_Graph::DelEdge(const int &SrcNid, const int &DstNid){
    ...
    AttrE.DelAttrDat(SrcNid,DstNid,false);
    EdgeNum--;
}

bool UNDirect_Weight_Graph::AddAttrDatE(const int &SrcNid, const int &DstNid, const int &AttrId, const void* Val,const uint& length){
    if(!IsEdge(SrcNid, DstNid)) return false;
    return AttrE.AddAttrDat(SrcNid, DstNid, AttrId, Val, length,false);
}
bool UNDirect_Weight_Graph::AddAttrDatE(const int &SrcNid, const int &DstNid, const char *AttrName, const void* Val,const uint& length){
    if(!IsEdge(SrcNid, DstNid)) return false;
    return AttrE.AddAttrDat(SrcNid, DstNid, AttrName, Val, length,false);
}
bool UNDirect_Weight_Graph::GetAttrDatE(const int &SrcNid, const int &DstNid, const int &AttrId, void *Val)const{
    if(!IsEdge(SrcNid, DstNid)) return false;
    return AttrE.GetAttrDat(SrcNid, DstNid, AttrId, Val);
}
bool UNDirect_Weight_Graph::GetAttrDatE(const int &SrcNid, const int &DstNid, const char *AttrName, void *Val)const{
    if(!IsEdge(SrcNid, DstNid)) return false;
    return AttrE.GetAttrDat(SrcNid, DstNid, AttrName, Val);
}
void UNDirect_Weight_Graph::DelAttrDatE(const int &SrcNid, const int &DstNid, const int &AttrId){
    if(IsEdge(SrcNid, DstNid)) AttrE.DelAttrDat(SrcNid, DstNid, AttrId,false);
}
void UNDirect_Weight_Graph::DelAttrDatE(const int &SrcNid, const int &DstNid, const char *AttrName){
    if(IsEdge(SrcNid, DstNid)) AttrE.DelAttrDat(SrcNid, DstNid, AttrName,false);
}
void UNDirect_Weight_Graph::DelAttrDatE(const int &SrcNid, const int &DstNid){
    if(IsEdge(SrcNid, DstNid)) AttrE.DelAttrDat(SrcNid, DstNid);
}


#endif /* UNDirect_Weight_Graph_h */
